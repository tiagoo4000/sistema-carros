...
const form = useForm({
    titulo: props.lote.titulo,
    descricao: props.lote.descricao,
    descricao_detalhada: props.lote.descricao_detalhada,
    valor_inicial: props.lote.valor_inicial,
    valor_minimo_incremento: props.lote.valor_minimo_incremento,
    valor_mercado: props.lote.valor_mercado,
    ordem: props.lote.ordem,
    status: props.lote.status,
    ano: props.lote.ano,
    quilometragem: props.lote.quilometragem,
    combustivel: props.lote.combustivel,
    cor: props.lote.cor,
    possui_chave: props.lote.possui_chave ? true : false,
    category_id: props.lote.category_id,
    tipo_retomada: props.lote.tipo_retomada,
    localizacao: props.lote.localizacao,
    prazo_documentacao: props.lote.prazo_documentacao,
    fotos: [],
    deleted_fotos: [],
    capa_tipo: null, // 'existing' or 'new'
    capa_valor: null,
});

const existingPhotos = ref(props.lote.fotos || []);

const visibleExistingPhotos = computed(() => {
    return existingPhotos.value.filter(p => !form.deleted_fotos.includes(p.id));
});
...
const removeNewFoto = (index) => {
    ...
    if (form.capa_tipo === 'new' && form.capa_valor === index) {
        form.capa_tipo = null;
        form.capa_valor = null;
    } else if (form.capa_tipo === 'new' && form.capa_valor > index) {
        form.capa_valor--;
    }
};

const removeExistingFoto = (id) => {
    form.deleted_fotos.push(id);
    if (form.capa_tipo === 'existing' && form.capa_valor === id) {
        form.capa_tipo = null;
        form.capa_valor = null;
    }
};

const isCapa = (type, value, photoPath = null) => {
    if (form.capa_tipo) {
        return form.capa_tipo === type && form.capa_valor === value;
    }
    if (type === 'existing' && props.lote.foto_capa && photoPath) {
        return props.lote.foto_capa === photoPath;
    }
    return false;
};

const setCapa = (type, value) => {
    form.capa_tipo = type;
    form.capa_valor = value;
};
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(route('admin.leiloes.lotes.update', [props.leilao.id, props.lote.id]));
};
</script>
            ...
            <div class="md:flex md:items-center md:justify-between mb-6">
                <div class="min-w-0 flex-1">
                    <h2 class="text-2xl font-bold leading-7 text-slate-900 sm:truncate sm:text-3xl sm:tracking-tight">
                        Editar Lote: {{ lote.titulo }}
                    </h2>
                    <p class="text-sm text-slate-500 mt-1">Leilão: {{ leilao.titulo }}</p>
                </div>
            </div>
                ...
                <div class="px-4 py-5 sm:p-6">
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <input type="text" :value="lote.subtitulo || leilao.titulo" disabled class="block w-full rounded-md border-slate-300 bg-slate-100 shadow-sm text-slate-500 sm:text-sm cursor-not-allowed" />
                            </div>
                        ...
                        <div class="border-t border-gray-200 pt-6">
                            ...
                            <!-- Fotos Existentes -->
                            <div v-if="visibleExistingPhotos.length > 0" class="mb-8">
                                <h4 class="text-sm font-medium text-slate-700 mb-2">Fotos Existentes</h4>
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                    <div v-for="foto in visibleExistingPhotos" :key="foto.id" class="relative group border rounded-lg p-2 bg-gray-50">
                                        <div class="aspect-w-16 aspect-h-12 w-full overflow-hidden rounded-md bg-gray-200 group-hover:opacity-75 h-32 flex items-center justify-center">
                                            <img :src="`/storage/${foto.caminho}`" class="h-full w-full object-cover object-center" />
                                        </div>
                                        
                                        <button @click.prevent="removeExistingFoto(foto.id)" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 focus:outline-none z-10">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                            </svg>
                                        </button>

                                        <div class="mt-2 flex items-center justify-center cursor-pointer" @click="setCapa('existing', foto.id)">
                                            <div :class="[
                                                'h-4 w-4 rounded-full border flex items-center justify-center',
                                                isCapa('existing', foto.id, foto.caminho) ? 'bg-emerald-600 border-emerald-600' : 'border-gray-300 bg-white'
                                            ]">
                                                <div v-if="isCapa('existing', foto.id, foto.caminho)" class="h-2 w-2 rounded-full bg-white"></div>
                                            </div>
                                            <span class="ml-2 block text-sm font-medium text-gray-700">Capa</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-slate-700 mb-2">Adicionar Novas Fotos (JPG, JPEG, WEBP)</label>
                                <input 
                                    type="file" 
                                    multiple 
                            ...
                            <div v-if="form.fotos.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-4">
                                <div v-for="(file, index) in form.fotos" :key="index" class="relative group border rounded-lg p-2 bg-gray-50">
                                    ...
                                    <button @click.prevent="removeNewFoto(index)" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 focus:outline-none z-10">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button>

                                    <div class="mt-2 flex items-center justify-center cursor-pointer" @click="setCapa('new', index)">
                                        <div :class="[
                                            'h-4 w-4 rounded-full border flex items-center justify-center',
                                            isCapa('new', index) ? 'bg-emerald-600 border-emerald-600' : 'border-gray-300 bg-white'
                                        ]">
                                            <div v-if="isCapa('new', index)" class="h-2 w-2 rounded-full bg-white"></div>
                                        </div>
                                        <span class="ml-2 block text-sm font-medium text-gray-700">Capa</span>
                                    </div>
                                </div>
